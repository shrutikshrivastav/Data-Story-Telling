...
def generate_visualizations(df, numeric_cols, categorical_cols):
    ...
    if numeric_cols:
        ...
        heatmap = ff.create_annotated_heatmap(
            z=corr.values, x=list(corr.columns), y=list(corr.index), colorscale="Viridis"
        )
...
def generate_narrative(df, summary, numeric_cols, categorical_cols):
    insights = []
    insights.append(f"Dataset cleaned: {summary['after_rows']} rows, {summary['after_cols']} columns.")
    insights.append(f"Duplicates removed: {summary['duplicates_removed']}, Missing fixed: {summary['missing_values_fixed']}, Outliers: {summary['outliers_detected']}.")
    insights.append(f"Quality score: {summary['quality_score']}%.")
    if numeric_cols:
        for col in numeric_cols:
            insights.append(f"'{col}' ranges {df[col].min():.2f}–{df[col].max():.2f}, avg {df[col].mean():.2f}.")
    if categorical_cols:
        for col in categorical_cols:
            top_cat = df[col].value_counts().idxmax()
            insights.append(f"'{col}' dominated by '{top_cat}'.")
...
def upload():
    ...
    return jsonify({"summary": summary, "visualizations": visualizations, "narrative": narrative})

@app.route("/download", methods=["POST"])
def download():
    file = request.files["file"]
    df = pd.read_csv(file) if file.filename.endswith(".csv") else pd.read_excel(file)
    cleaned_df, _, _, _ = clean_data(df)
    output = io.BytesIO()
    cleaned_df.to_csv(output, index=False)
    output.seek(0)
    return send_file(output, mimetype="text/csv", as_attachment=True, download_name=f"cleaned_{file.filename}")
